...
public:
    ...
    ImGuiKnob(Knob_Closure* kc, ImGuiNuke* t, const char* n) : Knob(kc, n)
    {
        theOp = t;
    }
...
void ImGuiNuke::DestroyFontsTexture()
{
    if (font_texture_)
    {
...
void ImGuiNuke::DestroyDeviceObjects()
{
...
void ImGuiNuke::Init(unsigned int width, unsigned int height)
{
    if (context_ == NULL)
    {
...
void ImGuiNuke::ImGuiKnobs(Knob_Callback f)
{
    CustomKnob1(ImGuiKnob, f, this, "kludge");
}

// OpenGL3 Render function.
// (this used to be set in io.RenderDrawListsFn and called by ImGui::Render(), but you can now call this directly from your main loop)
// Note that this implementation is little overcomplicated because we are saving/setting up/restoring every OpenGL state explicitly, in order to be able to run within any OpenGL engine that doesn't do so.
void ImGuiNuke::RenderDrawData(ImDrawData* draw_data)
{
